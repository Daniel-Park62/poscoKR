...
public class RegMote {
	...
	private static String[] TYPES = {"S","R"} ;
...
//    URL url8 = FileLocator.find(bundle, new Path("images/slice_page5.png"), null);
	...
	public RegMote(Composite parent, int style) {
		...
		Composite btncomp = new Composite(composite, SWT.BORDER) ;
		Button addbtn = new Button(btncomp, SWT.PUSH);
		addbtn.setFont(new Font(null, "Consolas", 16, SWT.NORMAL));
		addbtn.setText(" ADD ");
		addbtn.setLayoutData(new GridData(SWT.FILL, SWT.FILL, false, true));
		addbtn.pack();
		addbtn.addSelectionListener(new SelectionAdapter() {

			@Override
			public void widgetSelected(SelectionEvent e) {
				tempList.add(new MoteStatus(15) );
...
*/		
		...
		TableViewerColumn tcol = new TableViewerColumn(tableViewer, SWT.NONE | SWT.CENTER);
		TableColumn tblclmnNewColumn1 = tcol.getColumn();
		tblclmnNewColumn1.setAlignment(SWT.RIGHT);
		tblclmnNewColumn1.setWidth(80);
		tblclmnNewColumn1.setText("SEQ");

		tcol = new TableViewerColumn(tableViewer, SWT.NONE);
		TableColumn tblclmnNewColumn3 = tcol.getColumn();
		...
		TextCellEditor te = new TextCellEditor(table, SWT.CENTER);
		te.setValidator(new ICellEditorValidator() {
			
			@Override
			public String isValid(Object arg0) {
				String valueToValidate = tblclmnNewColumn8.getText() ;
				if (valueToValidate != null)
				try {
					Timestamp.valueOf(valueToValidate) ;
				} catch (Exception e2) {
					MessageBox dialog =  new MessageBox(parent.getShell() , SWT.ICON_ERROR | SWT.CLOSE);
					dialog.setText("��¥ ����");
					dialog.setMessage("��¥�Է��� �ٸ��� �ϼ���.");
					dialog.open();
					return "ERROR";
				}
...
//	    editors[5] = new ComboBoxCellEditor(table,TYPES,SWT.READ_ONLY);
	    editors[6] = new TextCellEditor(table);
	    editors[7] = new TextCellEditor(table);
	    
	    ((Text)editors[7].getControl()).addListener(SWT.Verify , new Listener() {
	    	@Override
	...
	String[] statusNm = {"Inactive","Sleep","Active"} ;
	  ...
	  public String getColumnText(Object element, int columnIndex) {
		  ...
		  if ( mote != null )
			  ...
			  case 7:
				  return (mote.getBattDt() == null || mote.getBattDt().length() < 8) ? mote.getBattDt()  : 
					  mote.getBattDt().substring(0, 4) + "-" + mote.getBattDt().substring(4, 6) + "-" + mote.getBattDt().substring(6) ;
			  }
		  return null;
	  }
